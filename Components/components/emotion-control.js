...
export default class EmotionControl extends Component {
  ...
  onPress = () => {
    this.setState({
      image: (this.state.index+1) % 8 == 0 ? require('../assets/color-selector.png') : require('../assets/color-wheel.png'),
      index: (this.state.index+1) % 8
    })
  }
    render() {
        return (
          <View>
          {/* <View style={{backgroundColor: this.state.list[this.state.index].color_val, flex: 1, opacity: this.state.list[this.state.index].opacity_val, height: 20, width: 20}}> */}
